...
export default function ObservationsScreen() {
  ...
  const [turtleDetails, setTurtleDetails] = useState<TurtleDetail[]>([
    createTurtleDetail(),
  ]);
  ...
  const [activeActivityDropdown, setActiveActivityDropdown] = useState<
    number | null
  >(null);
  ...
  const formattedSeenAt = useMemo(
    () =>
      seenAt.toLocaleString(undefined, {
        dateStyle: "medium",
        timeStyle: "short",
      }),
    [seenAt],
  ...
  const syncTurtleDetailCount = (desiredCount: number) => {
    setTurtleDetails((prev) => {
      ...
      if (desiredCount > prev.length) {
        return [
          ...prev,
          ...Array.from({ length: desiredCount - prev.length }, () =>
            createTurtleDetail(),
          ),
        ];
      }
    ...
    setActiveActivityDropdown((prev) =>
      prev !== null && prev >= desiredCount ? null : prev,
    );
  };
  ...
  const updateTurtleDetail = (
    index: number,
    partial: Partial<TurtleDetail>,
  ) => {
    setTurtleDetails((prev) =>
      prev.map((detail, detailIndex) =>
        detailIndex === index ? { ...detail, ...partial } : detail,
      ),
    );
  };
    ...
    setTurtleDetails((prev) =>
      prev.map((detail, detailIndex) => {
        ...
        return {
          ...
          activities: {
            ...detail.activities,
            [option]: !detail.activities[option],
          },
        };
      }),
    );
  ...
  const getActivitiesLabel = (detail: TurtleDetail) => {
    const selected = ACTIVITY_OPTIONS.filter(
      (option) => detail.activities[option],
    ).map(activityLabel);
    if (!selected.length) {
  ...
  const handleSubmit = async () => {
    if (!user) {
      Alert.alert(
        t("observations.alert.signIn.title"),
        t("observations.alert.signIn.body"),
      );
      return;
    }
    ...
    if (!photos.length) {
      Alert.alert(
        t("observations.alert.photos.title"),
        t("observations.alert.photos.body"),
      );
      return;
    }
    ...
    if (!latitude || !longitude) {
      Alert.alert(
        t("observations.alert.location.title"),
        t("observations.alert.location.body"),
      );
      return;
    }
    ...
    if (Number.isNaN(countValue) || countValue <= 0) {
      Alert.alert(
        t("observations.alert.count.title"),
        t("observations.alert.count.body"),
      );
      return;
    }
    ...
    if (turtleDetails.length !== countValue) {
      Alert.alert(
        t("observations.alert.turtleMismatch.title"),
        t("observations.alert.turtleMismatch.body"),
      );
      return;
    }
    ...
    if (actionTaken === "Other" && !actionOther.trim()) {
      Alert.alert(
        t("observations.alert.actionOther.title"),
        t("observations.alert.actionOther.body"),
      );
      return;
    }
    ...
    try {
      ...
      const observationsToInsert = turtleDetails.map((detail, index) => {
        const selectedActivities = ACTIVITY_OPTIONS.filter(
          (option) => detail.activities[option],
        );
      ...
      <ScrollView contentContainerStyle={styles.container}>
        <View style={styles.hero}>
          ...
          <Text style={styles.subtitle}>
            {t("observations.hero.subtitle", { name: displayName })}
          </Text>
        </View>
        ...
        <View style={styles.step}>
          <View style={styles.stepHeader}>
            <Text style={styles.stepBadge}>
              {t("observations.stepLabel", { number: 1 })}
            </Text>
            <Text style={styles.stepTitle}>
              {t("observations.step1.title")}
            </Text>
          </View>
          <Text style={styles.stepInstruction}>
            {t("observations.step1.instruction")}
          </Text>
          <Text style={styles.stepSource}>{t("observations.step.source")}</Text>
          <View style={styles.warningBox}>
            <Text style={styles.warningTitle}>
              {t("observations.step1.warningTitle")}
            </Text>
            <Text style={styles.warningCopy}>
              {t("observations.step1.warningCopy")}
            </Text>
            <Text style={styles.stepSource}>
              {t("observations.step.source")}
            </Text>
          </View>
            ...
            >
              <Text
                style={[
                  ...
                  styles.primaryUploadButtonText,
                ]}
              >
                {isPickingPhoto
                  ? t("observations.photos.take")
                  : t("observations.photos.take")}
              </Text>
            </Pressable>
          </View>
        ...
        <View style={styles.step}>
          <View style={styles.stepHeader}>
            <Text style={styles.stepBadge}>
              {t("observations.stepLabel", { number: 2 })}
            </Text>
            <Text style={styles.stepTitle}>
              {t("observations.step2.title")}
            </Text>
          </View>
          <Text style={styles.stepInstruction}>
            {t("observations.step2.instruction")}
          </Text>
          <Text style={styles.stepSource}>{t("observations.step.source")}</Text>
          <Pressable
          ...
          >
            {isLocating ? (
              ...
              <Text style={styles.secondaryButtonText}>
                {t("observations.location.useCurrent")}
              </Text>
            )}
          </Pressable>
          <View style={styles.fieldGroup}>
            <View style={styles.fieldRow}>
              <View style={styles.fieldHalf}>
                <Text style={styles.label}>
                  {t("observations.location.latitude")}
                </Text>
                <TextInput
              ...
              <View style={styles.fieldHalf}>
                <Text style={styles.label}>
                  {t("observations.location.longitude")}
                </Text>
                <TextInput
            ...
            <View>
              <Text style={styles.label}>
                {t("observations.location.nameLabel")}
              </Text>
              <TextInput
        ...
        <View style={styles.step}>
          <View style={styles.stepHeader}>
            <Text style={styles.stepBadge}>
              {t("observations.stepLabel", { number: 3 })}
            </Text>
            <Text style={styles.stepTitle}>
              {t("observations.step3.title")}
            </Text>
          </View>
          <Text style={styles.stepInstruction}>
            {t("observations.step3.instruction")}
          </Text>
          <Text style={styles.stepSource}>{t("observations.step.source")}</Text>
          ...
          {turtleDetails.map((detail, index) => {
            const hasSelectedActivities = ACTIVITY_OPTIONS.some(
              (option) => detail.activities[option],
            );
            return (
              <View key={`turtle-${index}`} style={styles.turtleCard}>
                ...
                <View>
                  <Text style={styles.label}>
                    {t("observations.turtle.activityLabel", {
                      number: index + 1,
                    })}
                  </Text>
                  <Pressable
                    style={[
                      styles.dropdown,
                      activeActivityDropdown === index && styles.dropdownOpen,
                    ]}
                  ...
                  >
                    <Text
                      style={[
                        styles.dropdownText,
                        !hasSelectedActivities && styles.dropdownPlaceholder,
                      ]}
                    >
                ...
                <View>
                  ...
                  <TextInput
                    ...
                    value={detail.notes}
                    onChangeText={(text) =>
                      updateTurtleDetail(index, { notes: text })
                    }
                  />
                </View>
              </View>
            ...
            {ACTION_OPTIONS.map((option) => (
              ...
              >
                <View
                  style={[
                    styles.radioOuter,
                    actionTaken === option && styles.radioSelected,
                  ]}
                >
                  {actionTaken === option ? (
                    <View style={styles.radioInner} />
        ...
        <View style={styles.step}>
          <Text style={styles.stepTitle}>
            {t("observations.additional.title")}
          </Text>
          <TextInput
            multiline
            numberOfLines={5}
